(() => {
  ...
  function renderStudy(study) {
    ...
    const targetSummary=classification?`<div><span>Positive class</span><strong>${pct(study.dataset.positive_rate)}</strong><small>${num(positiveCount)} ${esc(study.dataset.positive_label||"high")} records</small></div>`:`<div><span>Target median</span><strong>${formatValue(study,study.dataset.target_summary.median)}</strong><small>range ${formatValue(study,study.dataset.target_summary.minimum)}–${formatValue(study,study.dataset.target_summary.maximum)}</small></div>`;
