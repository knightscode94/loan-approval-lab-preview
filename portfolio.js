(() => {
  ...
  const formatValue = (study, value, digits = 0) => {
    const unit = study.dataset.unit || "currency";
    if (unit === "percentage_points") return `${Number(value).toLocaleString("en-GB", {maximumFractionDigits: digits || 2})} pp`;
    if (unit === "score_points") return `${Number(value).toLocaleString("en-GB", {maximumFractionDigits: digits || 2})} pts`;
  ...
  function multiModelChart(study) {
    ...
    return bars(rows, {left:220, max:Math.max(...rows.map(row=>row.value))*1.12, format:value=>classification?dec(value):formatValue(study,value), aria:`${study.title} model comparison`});
  }
  function importanceChart(study) {
    ...
    return bars(rows, {left:230,rowHeight:34,max:Math.max(...rows.map(row=>row.value),.001)*1.1,format:value=>study.type==="regression"?formatValue(study,value):dec(value,3),aria:`${study.title} permutation importance`});
  }
  ...
  function residualChart(study) {
    const rows=study.test.residual_bins.map(item=>({label:formatValue(study,item.actual),value:item.mae}));
    return bars(rows,{left:135,rowHeight:34,max:Math.max(...rows.map(row=>row.value))*1.15,format:value=>formatValue(study,value),aria:`${study.title} error by target band`});
  }
  ...
  const overviewItems = [
    ...
    ...allStudies.map(study => study.type === "classification" ? {id:study.id,title:study.title,type:"Classification",model:study.selection.model,score:study.test.metrics.average_precision,scoreLabel:"Average precision",display:dec(study.test.metrics.average_precision),note:`Prevalence ${dec(study.dataset.positive_rate)} · ROC AUC ${dec(study.test.metrics.roc_auc)}`,tone:study.test.metrics.average_precision-study.dataset.positive_rate>.08?"strong":"warning"} : {id:study.id,title:study.title,type:"Regression",model:study.selection.model,score:study.test.metrics.r2,scoreLabel:"Test R²",display:dec(study.test.metrics.r2),note:`RMSE ${formatValue(study,study.test.metrics.rmse)} · MAE ${formatValue(study,study.test.metrics.mae)}`,tone:study.test.metrics.r2>0?"strong":"warning"})
  ];
  ...
  function metricCells(study) {
    ...
    return `<div><span>RMSE</span><strong>${formatValue(study,metrics.rmse)}</strong><small>penalises large errors</small></div><div><span>MAE</span><strong>${formatValue(study,metrics.mae)}</strong><small>typical absolute error</small></div><div><span>R²</span><strong>${dec(metrics.r2)}</strong><small>variance explained</small></div><div><span>${esc(study.relative_metric_label||"MAPE")}</span><strong>${pct(metrics.mape)}</strong><small>relative error</small></div>`;
  }
  function comparisonTable(study) {
    ...
    const rows=study.comparison.map(item=>`<tr class="${item.model===study.selection.model?"selected":""}"><td>${esc(item.model)}</td>${classification?`<td>${dec(item.average_precision)}</td><td>${dec(item.roc_auc)}</td><td>${dec(item.f1)}</td><td>${dec(item.precision)}</td><td>${dec(item.recall)}</td>`:`<td>${formatValue(study,item.rmse)}</td><td>${formatValue(study,item.mae)}</td><td>${dec(item.r2)}</td><td>${pct(item.mape)}</td>`}</tr>`).join("");
  ...
  function renderStudy(study) {
    const classification=study.type==="classification", primary=classification?dec(study.test.metrics.average_precision):formatValue(study,study.test.metrics.rmse), primaryLabel=classification?"test average precision":"test RMSE";
    ...
    const targetSummary=classification?`<div><span>Positive class</span><strong>${pct(study.dataset.positive_rate)}</strong><small>${num(positiveCount)} ${esc(study.dataset.positive_label||"high")} records</small></div>`:`<div><span>Target median</span><strong>${formatValue(study,study.dataset.target_summary.median)}</strong><small>range ${formatValue(study,study.dataset.target_summary.minimum)} to ${formatValue(study,study.dataset.target_summary.maximum)}</small></div>`;
    const bestParams=Object.entries(study.tuning.best_parameters).map(([key,value])=>`<span>${esc(label(key))}: ${esc(value)}</span>`).join("");
    const cvScore=classification?dec(study.tuning.best_cv_score):formatValue(study,study.tuning.best_cv_score);
