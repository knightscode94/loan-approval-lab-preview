(() => {
  ...
  const studies = Object.fromEntries(portfolio.studies.map(study => [study.id, study]));
  ...
  const overviewItems = [
    ...
    ...portfolio.studies.map(study => study.type === "classification" ? {id:study.id,title:study.title,type:"Classification",model:study.selection.model,score:study.test.metrics.average_precision,scoreLabel:"Average precision",display:dec(study.test.metrics.average_precision),note:"Leakage-free fields carry no reliable predictive signal.",tone:"warning"} : {id:study.id,title:study.title,type:"Regression",model:study.selection.model,score:study.test.metrics.r2,scoreLabel:"Test R²",display:dec(study.test.metrics.r2),note:`RMSE ${money(study.test.metrics.rmse)} · MAE ${money(study.test.metrics.mae)}`,tone:"strong"})
  ];
  ...
  function evaluationVisuals(study) {
    if(study.type==="classification") {
      ...
      return `<div class="study-two"><div class="study-panel"><h3>Confusion matrix</h3><div class="mini-confusion"><div class="good"><strong>${num(matrix.tn)}</strong><span>True low</span></div><div class="bad"><strong>${num(matrix.fp)}</strong><span>False high</span></div><div class="bad"><strong>${num(matrix.fn)}</strong><span>Missed high</span></div><div class="good"><strong>${num(matrix.tp)}</strong><span>True high</span></div></div></div><div class="study-panel"><h3>ROC & precision–recall curves</h3><div class="generic-chart">${curve}</div></div></div>`;
    }
  ...
  function renderStudy(study) {
    ...
    const targetSummary=classification?`<div><span>Positive class</span><strong>${pct(study.dataset.positive_rate)}</strong><small>${num(study.dataset.target_counts.High)} high records</small></div>`:`<div><span>Target median</span><strong>${money(study.dataset.target_summary.median)}</strong><small>range ${money(study.dataset.target_summary.minimum)}–${money(study.dataset.target_summary.maximum)}</small></div>`;
    ...
    <div class="study-body">
      <section class="study-stage"><div class="stage-head"><span>01</span><div><p class="eyebrow">Problem & approach</p><h2>Define the decision before the model</h2></div></div><p class="stage-copy">${esc(study.business_question)} This is framed as supervised ${study.type}, with <code>${esc(study.target)}</code> as the target and ${esc(study.primary_metric)} as the primary selection metric.</p>${leakage}<div class="study-two"><div class="study-panel"><h3>Business use</h3><p>${classification?"Use probability-ranked records for planning and review—not as a fabricated guarantee of future sales.":study.id==="housing"?"Support district-level planning, scenario comparison, and valuation review—not individual property appraisal.":"Support budgeting and anomaly review while keeping tariff and engineering judgement outside the model."}</p></div><div class="study-panel"><h3>Candidate families</h3><p>${classification?"Dummy prior, logistic regression, random forest, and histogram gradient boosting.":"Median baseline, ridge regression, random forest, and histogram gradient boosting."}</p></div></div></section>
      <section class="study-stage"><div class="stage-head"><span>02</span><div><p class="eyebrow">Data audit</p><h2>Know what the table can support</h2></div></div><div class="fact-grid"><div><span>Rows</span><strong>${num(study.dataset.rows)}</strong><small>source records</small></div><div><span>Missing cells</span><strong>${num(study.dataset.missing_total)}</strong><small>handled in-pipeline</small></div><div><span>Duplicates</span><strong>${num(study.dataset.duplicate_rows)}</strong><small>exact rows</small></div>${targetSummary}</div><div class="study-two"><div class="study-panel"><h3>Largest IQR outlier rates</h3><div class="generic-chart">${bars([...study.dataset.outliers_iqr].sort((a,b)=>b.rate-a.rate).slice(0,6).map(item=>({label:label(item.feature),value:item.rate})),{left:190,format:value=>pct(value),max:Math.max(...study.dataset.outliers_iqr.map(item=>item.rate),.01)*1.1})}</div></div><div class="study-panel"><h3>Audit interpretation</h3><p>${study.id==="bmw"?"No missing values or duplicates. The target imbalance is moderate, but the decisive issue is its deterministic dependence on realised volume.":study.id==="housing"?"207 bedroom counts are missing and the target is capped at $500,001. Aggregate count features are strongly right-skewed.":"No missing rows or duplicates were found. Source spelling errors were standardised without changing values."}</p></div></div></section>
      <section class="study-stage"><div class="stage-head"><span>03</span><div><p class="eyebrow">Preparation & partition</p><h2>Fit transformations on training data only</h2></div></div><p class="stage-copy">${esc(study.preparation.feature_selection)}</p><div class="study-two"><div class="study-panel"><h3>Included features</h3><div class="feature-chips">${[...study.preparation.numeric_features,...study.preparation.categorical_features].map(feature=>`<span>${esc(feature)}</span>`).join("")}${(study.preparation.excluded_features||[]).map(feature=>`<span class="excluded">${esc(feature)}</span>`).join("")}</div></div><div class="study-panel"><h3>Pipeline</h3><ol>${study.preparation.steps.map(step=>`<li>${esc(step)}</li>`).join("")}</ol></div></div><div class="fact-grid"><div><span>Train</span><strong>${num(study.split.train.rows)}</strong><small>70% · fit</small></div><div><span>Validation</span><strong>${num(study.split.validation.rows)}</strong><small>15% · select</small></div><div><span>Test</span><strong>${num(study.split.test.rows)}</strong><small>15% · open once</small></div><div><span>Strategy</span><strong>${classification?"Stratified":"Seeded"}</strong><small>fixed random state</small></div></div></section>
  ...
  portfolio.studies.forEach(study => { document.getElementById(`study-${study.id}`).innerHTML=renderStudy(study); });
})();
